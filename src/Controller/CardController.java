...
public class CardController implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source instanceof CardView) {
            ...
            System.out.println("Card clicked - In discard mode: " + isDiscardMode); // Debug info
            if (isDiscardMode) {
    ...
    private void handleCardClick(Card card) {
        System.out.println(
                "Handling card click - Card type: " + (card != null ? card.getClass().getSimpleName() : "null")); // Debug
                                                                                                                  // info
    ...
    private void handleHelicopterCard(HelicopterCard card) {
        ...
        // Show player selection dialog (multiple selection)
        ...
        while (true) {
            ...
            for (int i = 0; i < currentOptions.length; i++) {
                ...
                playerButtons[i].addActionListener(e -> {
                    ...
                    playerButtons[index]
                            .setText(playerOptions[index] + String.format(" (Selected %d times)", selectedCount));
                });
    ...
    public void enableDiscardMode(PlayerInfoView playerInfoView, int numCardsToDiscard) {
        ...
        JPanel cardsPanel = playerInfoView.getCardsPanel();
        System.out.println("Current number of components in card panel: " + cardsPanel.getComponentCount()); // Debug
                                                                                                             // info
        ...
        for (Component component : cardsPanel.getComponents()) {
            if (component instanceof CardView) {
                ...
                System.out.println("Enabled card click event: " + cardView.getCard().getClass().getSimpleName()); // Debug
                                                                                                                  // info
            }
        }
    ...
    private void handleDiscardCard(Card card) {
        System.out
                .println("Handling discard - Currently discarded " + cardsDiscarded + "/" + cardsToDiscard + " cards"); // Debug
                                                                                                                        // info
        if (cardsDiscarded < cardsToDiscard) {
            ...
            if (cardsDiscarded == cardsToDiscard) {
                ...
                // Check if discard was caused by giving cards
                ...
                JOptionPane.showMessageDialog(null,
                        "Need to discard " + (cardsToDiscard - cardsDiscarded) + " more cards",
                        "Discard Phase",
                        JOptionPane.INFORMATION_MESSAGE);
            }
        }
    ...
    public boolean giveCard(int fromPlayerIndex, int toPlayerIndex, Card card) {
        ...
        if (cardCount > 5) {
            ...
            // If not exceeding limit, still consume A's action point
            if (pendingGiveCardPlayerIndex != null) {
                int aIndex = pendingGiveCardPlayerIndex;
                PlayerInfoView aView = gameController.getPlayerInfoView(aIndex);
                String actionText = aView.getActionPointsLabel().getText();
    ...
    public boolean useSandbagCard(int playerIndex, Tile targetTile) {
        ...
        if (((SandbagCard) sandbagCard).useCard(targetTile)) {
            ...
            System.out.println("[Log] Successfully used sandbag card to shore up tile: " + targetTile.getName()
                    + " [Coordinates: " + targetTile.getRow() + ","
                    + targetTile.getCol() + "]");
            return true;
        } else {
            System.out.println("[Log] Sandbag card shoring up failed");
            return false;
        }
    }
    ...
    public boolean useHelicopterCardForWin(int playerIndex) {
        // 1. Check if all treasures are collected
        if (!gameController.getTreasureDeck().allTreasuresCollected()) {
            ...
            JOptionPane.showMessageDialog(null, "Not all treasures have been collected, cannot escape!");
            return false;
        }
     ...
     * 
     * @return Returns true if in discard mode
     */
     ...
     * Call this method after special card use is complete to continue discard
     */
    public void continueDiscardMode() {
        if (isDiscardMode && currentDiscardingPlayer != null) {
            ...
            if (cardsDiscarded < cardsToDiscard) {
                ...
                currentDiscardingPlayer.setButtonsEnabled(true);
                currentDiscardingPlayer = null;
                // Fix: If there is pendingGiveCardPlayerIndex, consume A's action point and
                // determine whether to return to A or switch turns
                if (pendingGiveCardPlayerIndex != null) {
                    int aIndex = pendingGiveCardPlayerIndex;
                    PlayerInfoView aView = gameController.getPlayerInfoView(aIndex);
                    String actionText = aView.getActionPointsLabel().getText();
